...
let first_constraint grammer nulls=
  ...
  let constraints=List.map (fun x->{sym=x;set=ref []}) terminals in 
...
let rec first_one set constraints=match set with
  | []->[]
  | h::t->(match h with
    | T(_) -> let sym_constr=List.find (fun x->x.sym=h) constraints in
      !(sym_constr.set)@(first_one t constraints)
    | NT(_)->h::(first_one t constraints)
...
let calc_firsts grammer nulls=let constraints=first_constraint grammer nulls in 
  ...
  | h::t->let prev= !(h.set) in
    let new_set=first_one !(h.set) constraints in h.set:=new_set;
...
let rec follow_one set constraints firsts=match set with
  | []->[]
  | h::t->(match h with
    | First(sym)->let first=List.find (fun x->x.sym=sym) firsts in 
      (List.map (fun x->Sym(x)) !(first.set))@(follow_one t constraints firsts)
    | Follow(sym)->let constr=List.find (fun x->x.sym=sym) constraints in 
      !(constr.set)@(follow_one t constraints firsts)
    | Sym(_)->h::(follow_one t constraints firsts))
...
let calc_follows grammer=
  ...
  | [] -> is_change
  | h::t->let prev= !(h.set) in 
    let new_set=follow_one !(h.set) constraints firsts in h.set:=new_set;
    if prev=new_set then one_loop is_change t else one_loop true t 
  in let is_change=ref true in 
  while !is_change=true do
    is_change:=one_loop false constraints 
  done;
  let to_syms=List.map (fun x->match x with
    | Sym(s) -> s
